...
const NamePage = ({ onNext, onBack, nativeLang }) => {
    ...
    const h = isFrench ? "Comment t'appelles-tu ? 👨🏽👧🏽" : "What is your name? 👨🏽👧🏽";
    const lbl = isFrench ? "Nom complet" : "Full Name";
    const ph  = isFrench ? "ex. Jean Dupont" : "e.g. John Doe";
        ...
        <div style={{ width: '100%', minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#fff' }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', width: '100%', maxWidth: '500px', margin: '0 auto', padding: '2rem 1.5rem 2.5rem' }}>
                ...
                <button
                    ...
                    disabled={!ok}
                    style={{
                        width: '100%', backgroundColor: ok ? B : '#cbd5e1', color: '#fff',
                        padding: '1.1rem', borderRadius: '9999px', fontSize: '1.05rem',
                        fontWeight: '700', border: 'none', cursor: ok ? 'pointer' : 'not-allowed',
                        boxShadow: ok ? `0 8px 24px rgba(27,79,216,0.3)` : 'none',
                        letterSpacing: '0.3px',
                    }}
                >{btn}</button>
            </div>
